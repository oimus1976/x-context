...
class FR004LikesTests(unittest.TestCase):
    ...
    def cli(self, transport, args=(), environ=None):
        ...
        code = main(['likes', *args], environ={'X_CONTEXT_USER_ACCESS_TOKEN': 'fake-user-token'} if environ is None else environ,
                    transport=transport, stdout=out, stderr=err)
        return code, out.getvalue(), json.loads(err.getvalue())

    def test_FR_004_official_endpoint_subject_binding_and_canonical(self):
        ...
        self.assertEqual((parsed.scheme, parsed.netloc, parsed.path), ('https', 'api.x.com', '/2/users/42/liked_tweets'))
        ...
        self.assertEqual(value['operation'], 'likes')
    ...
    def test_FR_004_user_credential_only(self):
    ...
    def test_FR_004_default_and_bounds(self):
    ...
    def test_FR_004_invalid_input_before_transport(self):
        ...
        for token in ('', 'fake\nunsafe', 3):
            ...
            with self.assertRaises(x_api.XApiError) as caught:
                x_api.lookup_likes(user_access_token='fake-user', page_token=token, transport=transport)
            self.assertEqual(caught.exception.category, 'invalid_input')
            self.assertEqual(transport.requests, [])
        for value in (True, 0, 101, 2.5, '25', None):
            transport = self.transport()
            with self.assertRaises(x_api.XApiError) as caught:
                x_api.lookup_likes(user_access_token='fake-user', max_results=value, transport=transport)
            self.assertEqual(caught.exception.category, 'invalid_input')
            self.assertEqual(transport.requests, [])

    def test_FR_004_binding_failure_stops_collection(self):
        transport = self.transport()
    ...
    def test_FR_004_one_page_continuation_and_completeness(self):
        for count in (0, 1, 25):
    ...
    def test_FR_004_malformed_success_fails_closed(self):
    ...
    def test_FR_004_conservative_errors_and_accounting(self):
    ...
    def test_FR_004_private_values_excluded_from_diagnostics_and_repr(self):
        ...
        result = x_api.lookup_likes(user_access_token='fake-user-token', page_token='fake-input-secret', transport=transport)
    ...
    def test_FR_004_transport_failure_redacted(self):
        for exception_type in (RuntimeError, x_api.XApiError):
            for stage in (0, 1):
                transport = self.transport()
                transport.replies[stage] = exception_type('fake-transport-secret')
                try:
                    x_api.lookup_likes(user_access_token='fake-user-token', transport=transport)
    ...
    def test_FR_004_no_default_persistence(self):
    ...
    def test_FR_004_no_target_or_all_cli(self):
        for args in (['42'], ['--user-id', '42'], ['--all'], ['--token', 'fake-secret'], ['--max-r', '1']):
            transport = self.transport()
            code, out, diag = self.cli(transport, args)
            self.assertEqual((code, out), (2, ''))
            self.assertEqual(transport.requests, [])
            self.assertEqual(diag['operation'], 'likes')

    def test_FR_004_rate_metadata_for_both_attempts(self):
        transport = self.transport()
        ...
        self.assertEqual(diag['rate_limits']['likes']['remaining'], 3)
        self.assertIsNone(diag['rate_limits']['likes']['reset'])
    ...
    def test_FR_004_overfull_page_fails_closed(self):
        transport = self.transport({'data': [{'id': '1', 'text': 'fake'}, {'id': '2', 'text': 'fake'}]})
        code, out, diag = self.cli(transport, ['--max-results', '1'])
        self.assertEqual((code, out, diag['error_category']), (3, '', 'provider_error'))
        self.assertEqual(diag['provider_requests_attempted'], 2)

    def test_FR_004_subject_failure_blocks_likes(self):
        for payload in ({}, {'data': {'id': 'bad'}}, {'data': {'id': '42'}, 'errors': [{}]}):
            transport = ScriptedTransport([response(payload)])
            code, out, diag = self.cli(transport)
            self.assertEqual((code, out, diag['error_category']), (3, '', 'provider_error'))
            self.assertEqual(len(transport.requests), 1)
            self.assertEqual(diag['provider_requests_attempted'], 1)

    def test_FR_004_canonical_requires_subject(self):
        from datetime import datetime, timezone
        from x_context.canonical import CanonicalEnvelope, Page
        for subject in (None, {'id': '42'}, '42'):
            with self.assertRaises(ValueError):
                CanonicalEnvelope('likes', datetime.now(timezone.utc), subject, (), Page())
        envelope = CanonicalEnvelope('likes', datetime.now(timezone.utc), x_api.AuthenticatedSubject('42'), (), Page())
        self.assertEqual(envelope.to_dict()['subject'], {'id': '42'})

    def test_FR_004_default_transport_one_attempt_per_endpoint(self):
        from unittest.mock import MagicMock
        for stage in (0, 1):
            opener = MagicMock()
            replies = []
            for payload in ({'data': {'id': '42'}}, {'data': []}):
                reply = MagicMock()
            ...
            replies[stage] = RuntimeError('fake-transport-secret')
            opener.open.side_effect = replies
            with patch.object(x_api, 'build_opener', return_value=opener) as build:
                code, out, diag = self.cli(None)
            self.assertEqual((code, out, diag['error_category']), (3, '', 'provider_error'))
            self.assertEqual(opener.open.call_count, stage + 1)
            self.assertEqual(diag['provider_requests_attempted'], stage + 1)
            self.assertNotIn('fake-transport-secret', str(diag))
            self.assertTrue(all(isinstance(call.args[0], x_api._NoRedirect) for call in build.call_args_list))
            urls = [call.args[0].full_url for call in opener.open.call_args_list]
            self.assertEqual(urls[0], 'https://api.x.com/2/users/me')
            if stage == 1:
                self.assertEqual(urls[1], 'https://api.x.com/2/users/42/liked_tweets?max_results=25')

    def test_FR_004_redirects_not_followed(self):
